...
const Dashboard = ({ onLogout, isDark, toggleTheme }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (machineState === 'PRESSING' && Math.random() > 0.95) {
        vibration = 15;
        addAlert('High Vibration Detected', 'warning');
      }
      if (machineState === 'PRESSING' && currentPressure.current < TARGET_PRESSURE * 0.9) {
        addAlert('Pressure Loss: Check Hydraulics', 'critical');
      }
    ...
    <div className={`min-h-screen font-sans p-4 md:p-8 relative overflow-hidden theme-transition animate__animated animate__fadeIn
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <KpiCard 
          ...
          trend={dataStream.length > 0 ? ((dataStream[dataStream.length-1].temp - TARGET_TEMP)/TARGET_TEMP * 100).toFixed(1) : 0}
        />
        <KpiCard 
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="space-y-6">
          ...
          <GlassCard className="p-6" isDark={isDark}>
            ...
            <div className={`mt-2 text-xs text-center font-mono rounded-full py-1 border theme-transition
              ${isDark ? "text-cyan-400 bg-black/40 border-cyan-500/30" : "text-stone-500 bg-white bg-opacity-40 border-white"}`}>
              Peak: {dataStream.length > 0 ? dataStream[dataStream.length-1].vibration.toFixed(2) : 0} mm/s
            </div>
          </GlassCard>
          ...
          <GlassCard className="p-6 flex-1 min-h-[200px] flex flex-col" isDark={isDark}>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar flex-1">
              {alerts.length === 0 && (
                <div className={`text-xs italic text-center py-4 theme-transition ${isDark ? "text-cyan-800" : "text-stone-400"}`}>System Nominal</div>
              )}
              {alerts.map((alert, idx) => (
                <div key={idx} className={`p-3 rounded-lg text-xs flex items-center shadow-sm border theme-transition
                  ${alert.type === 'critical' 
                    ? (isDark ? 'bg-red-900/30 border-red-500/50 text-red-400' : 'bg-rose-50 border-rose-200 text-rose-700')
                    : (isDark ? 'bg-orange-900/30 border-orange-500/50 text-orange-400' : 'bg-amber-50 border-amber-200 text-amber-800')}`}>
                  <span className="font-mono opacity-60 mr-2 text-[10px]">
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6">
        <GlassCard className="p-5" isDark={isDark}>
          ...
          <div className={`space-y-2 text-sm theme-transition ${isDark ? "text-gray-400" : "text-stone-600"}`}>
            <div className={`flex justify-between border-b pb-1 theme-transition ${isDark ? "border-gray-800" : "border-stone-200 border-opacity-50"}`}><span>Set Temp</span> <span className={`font-mono font-bold ${isDark ? "text-gray-200" : "text-stone-800"}`}>{TARGET_TEMP}°C</span></div>
            <div className={`flex justify-between border-b pb-1 theme-transition ${isDark ? "border-gray-800" : "border-stone-200 border-opacity-50"}`}><span>Set Pressure</span> <span className={`font-mono font-bold ${isDark ? "text-gray-200" : "text-stone-800"}`}>{TARGET_PRESSURE} Bar</span></div>
            <div className="flex justify-between"><span>Cure Time</span> <span className={`font-mono font-bold ${isDark ? "text-gray-200" : "text-stone-800"}`}>15s</span></div>
          </div>
        </GlassCard>

        <GlassCard className="p-5" isDark={isDark}>
          <h3 className={`text-[10px] font-bold uppercase tracking-widest mb-3 theme-transition ${isDark ? "text-cyan-700" : "text-stone-400"}`}>Shift Efficiency</h3>
          <div className={`space-y-2 text-sm theme-transition ${isDark ? "text-gray-400" : "text-stone-600"}`}>
            <div className={`flex justify-between border-b pb-1 theme-transition ${isDark ? "border-gray-800" : "border-stone-200 border-opacity-50"}`}><span>Avg Cycle</span> <span className={`font-mono font-bold ${isDark ? "text-gray-200" : "text-stone-800"}`}>{lastCycleTime.toFixed(1)}s</span></div>
            <div className={`flex justify-between border-b pb-1 theme-transition ${isDark ? "border-gray-800" : "border-stone-200 border-opacity-50"}`}><span>OEE</span> <span className={`font-mono font-bold ${isDark ? "text-green-400" : "text-emerald-600"}`}>92%</span></div>
            <div className="flex justify-between"><span>Rejects</span> <span className={`font-mono font-bold ${isDark ? "text-red-400" : "text-rose-500"}`}>3</span></div>
          </div>
        </GlassCard>

        <GlassCard className="p-5 flex flex-col justify-center items-center text-center" isDark={isDark}>
           <div className={`text-[10px] uppercase tracking-widest mb-2 theme-transition ${isDark ? "text-cyan-700" : "text-stone-400"}`}>Predictive Health</div>
           <div className={`text-xl font-light mb-2 flex items-center theme-transition ${isDark ? "text-gray-200" : "text-stone-700"}`}>
             <Activity size={18} className={`mr-2 theme-transition ${isDark ? "text-green-400" : "text-emerald-500"}`}/>
             Optimal
           </div>
           <div className={`w-full h-1.5 rounded-full overflow-hidden theme-transition ${isDark ? "bg-gray-800" : "bg-stone-200"}`}>
             <div className="bg-gradient-to-r from-emerald-400 to-emerald-600 h-full w-[85%] rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)]"></div>
           </div>
           <div className={`text-[10px] mt-2 font-medium theme-transition ${isDark ? "text-gray-500" : "text-stone-500"}`}>Next Maint: 340 Cycles</div>
        </GlassCard>
      </div>
